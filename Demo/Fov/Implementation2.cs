...
public static class ShadowCast
{
    ...
    public static void ComputeVisibility(Map map, Point source, float viewRadius, List<Point> visiblePoints)
    {
        ...
        {
            CastLight(map, source, viewRadius, 1, 1.0f, 0.0f, s_octantTransform[txidx], visiblePoints);
        }
    }
    ...
    private static void CastLight(Map map, Point gridPosn, float viewRadius,
            int startColumn, float leftViewSlope, float rightViewSlope, OctantTransform txfrm, List<Point> visiblePoints)
    {
        ...
        int xDim = map.GetWidth();
        int yDim = map.GetHeight();
        ...
        for (int currentCol = startColumn; currentCol <= viewCeiling; currentCol++)
        {
            ...
            for (int yc = currentCol; yc >= 0; yc--)
            {
                ...
                if (distanceSquared <= viewRadiusSq)
                {
                    ...
                    visiblePoints.Add(new Point(gridX, gridY));
                }

                bool curBlocked = !map.Tiles[gridX][gridY].Transparent();
                    ...
                    if (curBlocked)
                    {
                        ...
                        if (leftBlockSlope <= leftViewSlope)
                        {
                            CastLight(map, gridPosn, viewRadius, currentCol + 1,
                                leftViewSlope, leftBlockSlope, txfrm, visiblePoints);
                        }
